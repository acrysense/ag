export default (root) => {
	...
	const scheduleDone = (callback = done) => {
		clearTimeout(transitionTimer)
		transitionTimer = setTimeout(callback, getDurMs())
	}
	...
	function close() {
		...
		cancelAnimationFrame(ariaFrame)
		ariaFrame = requestAnimationFrame(() => {
			panel.setAttribute('aria-hidden', 'true')
		})
	...
	return () => {
		...
		document.removeEventListener('pointerdown', onDocDown, true)
		document.removeEventListener('keydown', onDocKey, true)
		if (open._onResize) window.removeEventListener('resize', open._onResize)
