...
let main, thumbs, items = [], lastActive = null;
...
export const init = () => {
	...
	mql.addEventListener('change', () => { if (!root.hidden) mount(main?.activeIndex || 0); });
};
...
export const bindLightbox = (galleryRoot) => {
	...
	imgs.forEach((img, i) => {
		...
		img.addEventListener('click', (e) => { e.preventDefault(); open(sources, i); });
	});
};