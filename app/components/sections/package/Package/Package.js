...
export default (root) => {
	...
	const disposeProgress = initProgress(root)
	const disposeBlocks = initPackageBlocks(root)
...
function initProgress(root) {
	...
	return () => {
		...
		els.forEach((el) => delete el.__animated)
	}
}
...
function initPackageBlocks(root) {
	...
	tables.forEach((table) => {
		...
		const applyFilter = () => {
			...
			const regex = currentSearchTerm
				? new RegExp(`(${escapeRegExp(currentSearchTerm)})`, 'gi')
				: null
			...
			for (let i = 0; i < items.length; i++) {
				...
				if (
					el.classList.contains('n-package-table__row') &&
					!el.classList.contains('n-package-table__row-group')
				) {
					const parentRow = el
					...
					if (
						i + 1 < items.length &&
						items[i + 1].classList.contains('n-package-table__row-group')
					) {
						group = items[i + 1]
					}
					...
					const passWorkParent =
						currentWorkFilter === 'all' ||
						(currentWorkFilter === 'in-work' && workValue > 0)
					const passSearchParent = parentRow.textContent
						.toLowerCase()
					...
					if (group) {
						const childRows = Array.from(
							group.querySelectorAll('.n-package-table__row')
						)
						childRows.forEach((child) => {
							...
							const passWorkC =
								currentWorkFilter === 'all' ||
								(currentWorkFilter === 'in-work' && workValueC > 0)
							const passSearchC = child.textContent
								.toLowerCase()
								.includes(currentSearchTerm)
							...
							if (showChild) {
								...
								if (regex) {
									child
										.querySelectorAll('.n-package-table__value')
										.forEach((cell) => {
											cell.innerHTML = cell.dataset.original.replace(
												regex,
												'<strong>$1</strong>'
											)
										})
		...
		if (tabs.length) {
			tabs.forEach((tab) => {
				...
				disposers.push(() => tab.removeEventListener('click', onClick))
			})
		}
		...
		if (search) {
			...
			disposers.push(() => search.removeEventListener('input', onInput))
		}
